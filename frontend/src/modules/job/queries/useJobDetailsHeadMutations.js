...
import { bulkDelete, deleteItem, submitData, updateData } from '../services/jobDetailsHeadService'

export function useJobDetailsHeadMutations(moduleName, options = {}) {
  const queryClient = useQueryClient()

  const handleSuccess = (data, variables) => {
    toast.success(`${moduleName} operation successful`)
    queryClient.invalidateQueries(['job-list-details-heads'])
