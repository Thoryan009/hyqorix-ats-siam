...
import {
  ...
  submitData,
  updateData,
} from '../services/jobDetailsCategoryService'

export function useJobDetailsCategoryMutations(moduleName, options = {}) {
  const queryClient = useQueryClient()

  const handleSuccess = (data, variables) => {
    toast.success(`${moduleName} operation successful`)
    queryClient.invalidateQueries(['job-details-categories'])
