...
const BASE_URL = '/job-list-details'
...
export async function fetchDetailHeads() {
  const api = useApi()
  await api.sendRequest('job-list-detail-heads')
  return response(api)
}
...
export async function bulkCreate(payload) {
  const api = useApi()
  await api.sendRequest(`${BASE_URL}/bulk-create`, 'POST', payload)
  if (api.error.value) {
    throw api.error.value // 🚨 throw error so useMutation.onError triggers
  }
