...
import { submitData, updateData, deleteItem, bulkDelete } from '../services/subjectService'
import { toast } from '@/shared/config/toastConfig'

export function useSubjectMutations(moduleName, options = {}) {
  const queryClient = useQueryClient()

  const handleSuccess = (data, variables) => {
    toast.success(`${moduleName} operation successful`)
    queryClient.invalidateQueries(['subjects']) // Invalidate the subjects query to refetch updated data
