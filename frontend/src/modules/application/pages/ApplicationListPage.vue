<template>
  <SectionHeader>
    ...
    <PageHeader>
      <div>
        <!-- Responsive Heading -->
        <PageTitle>{{t('application.application_list')}}</PageTitle>
      </div>
    </PageHeader>
      ...
      <div>
        ...
        >
          ...
          <template #actions="{ row }">
            <BaseTableButton icon="fa fa-eye" :show-serial="false" variant="primary" title="View" @click="onView(row)" />
          </template>
        </BaseTable>
...
import { useApplicationListDataQuery, useApplicationsQuery } from '../queries/useApplicationsQuery'
...
const { data: applicationData } = useApplicationListDataQuery()
