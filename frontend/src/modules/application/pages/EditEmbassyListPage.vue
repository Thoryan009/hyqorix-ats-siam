<template>
  <SectionHeader>
    <PageHeader>
      <div>
        <PageTitle>{{ t('embassyList.edit') }}</PageTitle>
      </div>
    </PageHeader>

    <div v-if="isLoading" class="mt-4 rounded-lg bg-white p-6 shadow-sm">
      <p class="text-sm text-gray-500">{{ t('embassyList.loading_embassy_list') }}</p>
    </div>

    <div v-else class="mt-4 rounded-lg bg-white p-6 shadow-sm lg:p-8">
      <div class="mx-auto max-w-5xl rounded-md border border-gray-200 bg-white p-4 sm:p-6">
        ...
        <div class="mt-5 flex justify-center gap-3">
          <BaseButton
            ...
            :disabled="updateLoading || !hasApplicants"
            @click="handleUpdateList"
          >
            <span v-if="updateLoading">{{ t('shared.labels.updating') }}</span>
            <span v-else> {{ t('embassyList.update_list') }}</span>
          </BaseButton>
        </div>
      </div>
    </div>
...
const {
  ...
  buildPayload,
  loadFromRecord,
} = useEmbassyListForm({ excludeEmbassyListId: listId })
...
const { update, updateLoading } = useEmbassyListMutations('Embassy List', {
  onSuccess: () => {
