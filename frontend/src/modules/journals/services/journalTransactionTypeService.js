...
const BASE_URL = '/journal-transaction-types'
...
export async function fetchJournalTransactionTypeOptions(status = 'active') {
  const api = useApi()
  const url = buildUrl(`${BASE_URL}/options`, { status })
  await api.sendRequest(url)
  return response(api)
}
