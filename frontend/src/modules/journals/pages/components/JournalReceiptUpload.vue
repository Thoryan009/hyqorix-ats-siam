<template>
  <div class="space-y-3">
    ...
    <JournalReceiptGallery
      v-if="galleryItems.length"
      :items="galleryItems"
      @remove="removeReceiptAt"
    />
...
const galleryItems = computed(() => {
  const existing = props.existingUrls.map((src, index) => ({
  ...
  const uploaded = receiptPreviews.value.map((src, index) => ({
    key: `new-${index}-${src}`,
    src,
    alt: `${t('journals.receipt')} ${existing.length + index + 1}`,
    removable: true,
    removeIndex: index,
    file: props.receiptPath[index] instanceof File ? props.receiptPath[index] : null,
  }))
