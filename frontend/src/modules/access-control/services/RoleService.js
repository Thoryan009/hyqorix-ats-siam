...
const BASE_URL = '/access-control/roles'
...
export async function updateRolePermissionsData({ roleId, permissions }) {
  const api = useApi()
  await api.sendRequest(`${BASE_URL}/${roleId}/permissions`, 'PUT', { permission_ids: permissions })
  if (api.error.value) throw api.error.value
  return api.data.value
}
