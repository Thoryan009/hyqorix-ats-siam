<template>
  <div>
    <CreateExpenseCostAccountModal :cost-type="costType" />
    <EditExpenseCostAccountModal :cost-type="costType" />
            ...
            <template #actions="{ row }">
              <div class="flex items-center justify-center gap-1">
                <router-link
                  ...
                  :to="`/finance/expense-cost-accounts/${costType}/${row.id}/ledger`"
                >
                ...
                <BaseTableButton
                  ...
                  variant="success"
                  title="Edit"
                  @click="costStore.openEditModal(costType, row)"
                />
              </div>
            </template>
          </BaseTable>
...
import CreateExpenseCostAccountModal from './CreateExpenseCostAccountModal.vue'
import EditExpenseCostAccountModal from './EditExpenseCostAccountModal.vue'
...
const accounts = computed(() => costStore.getAccounts(props.costType))
...
const summaryCards = computed(() => [
  {
    ...
    subtitle: `${config.value.costTypeLabel} expense heads`,
    icon: 'fa fa-list',
    iconBg: 'bg-blue-50',
  ...
  },
  {
    ...
    subtitle: 'Accounts with ledger bills',
    icon: 'fa fa-arrow-up',
...
const quickActions = computed(() => [
  {
    label: `Create ${config.value.costTypeLabel} Account`,
    icon: 'fa fa-plus',
    onClick: () => costStore.openCreateModal(props.costType),
  },
  {
    label: 'Expense Setup',
    icon: 'fa fa-tags',
    onClick: () => router.push('/finance/expense-management'),
  },
  {
    label: 'Main Accounts',
    icon: 'fa fa-bank',
