<template>
  ...
  >
    <BaseForm v-if="account" :onSubmit="handleSubmit">
      <div class="space-y-2">
        <BaseLabel :for="`edit_${costType}_account_name`">Account Name</BaseLabel>
        <BaseInput
          :id="`edit_${costType}_account_name`"
...
watch(
  ...
  (isOpen) => {
    if (isOpen && costStore.activeCostType === props.costType && costStore.editingAccount) {
      form.account_name =
        costStore.editingAccount.account_name || costStore.editingAccount.head_name || ''
...
const handleSubmit = async () => {
  ...
  try {
    const result = await costStore.updateAccount(props.costType, account.value.id, {
    ...
    toast.success(`${config.value.costTypeLabel} account updated successfully`)
    closeModal()
  } finally {
    loading.value = false
  }
