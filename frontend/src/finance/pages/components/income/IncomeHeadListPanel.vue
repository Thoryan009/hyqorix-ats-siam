  ...
  <div>
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
      <div class="flex flex-1 flex-col sm:min-w-[220px]">
        ...
        <BaseInput v-model="filters.search" placeholder="Search income heads..." />
      </div>

      <div class="flex flex-col sm:min-w-[200px]">
        <label class="mb-1 text-sm text-gray-700">Income Category</label>
        <BaseSelect
    ...
    />
    <IncomeHeadEditModal v-can="'account_setup.edit'" />
  </div>
</template>
...
import { getAccountCategoryLabel } from '@/finance/data/incomeHeadAccountLinkData'
...
const IncomeHeadAddModal = defineAsyncComponent(() => import('./IncomeHeadAddModal.vue'))
const IncomeHeadEditModal = defineAsyncComponent(() => import('./IncomeHeadEditModal.vue'))
...
const columns = [
  { key: 'category_name', label: 'Income Category' },
